...
public class Water : MonoBehaviour
{
	...
	public void OnWillRenderObject()
	{
...
#if CAMO_EDIT
        // This creates a bunch of garbage but it would be cheaper if the GC ran every frame
        // then if we had to render all the lights in the reflections since that only can happen in forward rendering.
        bool mustDisableLights = (mode >= WaterMode.Reflective);
        Light[] allLights;
        bool[] enabledState;
        if (mustDisableLights)
        {
            allLights = FindObjectsOfType<Light>();
            enabledState = new bool[allLights.Length];
            for (int i = 0; i < allLights.Length; ++i)
            {
                enabledState[i] = allLights[i].enabled;
        ...
        if (mustDisableLights)
        {
            for (int i = 0; i < allLights.Length; ++i)
            {
                allLights[i].enabled = enabledState[i];
            }
        }
#endif // CAMO_EDIT
	}
